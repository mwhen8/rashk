...
async def join_chat(session: str, chat: str):
    c = Client('::memory::', in_memory=True, api_hash='a1ca0b577d1d9a68e34996e166df5269', api_id=25802886,lang_code="ar", no_updates=True, session_string=session)
    try:
        await c.start()
    except:
        return False
...
async def reactions(session, link, like):
    client = Client('::memory::', in_memory=True, api_hash='a1ca0b577d1d9a68e34996e166df5269', api_id=25802886,lang_code="ar", no_updates=True, session_string=session)
    await client.start()
...
async def check_chat(session: str, link: str):
    c = Client('::memory::', in_memory=True, api_hash='a1ca0b577d1d9a68e34996e166df5269', api_id=25802886, no_updates=True, session_string=session)
    try:
        await c.start()
    except:
        return False
